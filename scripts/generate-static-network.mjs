...
const isCapitalLocation = ({ latitude, longitude }) =>
...
const locationFor = (station) => {
  ...
  const capital = locations.filter(isCapitalLocation);
  if (!capital.length) return undefined;
  const lines = METADATA_LINES.get(station.line) ?? [station.line];
  const exact = capital.find(({ line }) => lines.includes(line));
  if (exact) return exact;
  if (capital.every((location) => distanceBetween(capital[0], location) < 1)) {
    return capital[0];
  }
...
const locations = new Map(
  [...stations].map(([id, station]) => [id, locationFor(station)]),
);
const isPhysicalNeighbor = (line, first, second) => {
  const firstLocation = locations.get(`${line}|${first}`);
  const secondLocation = locations.get(`${line}|${second}`);
  if (!firstLocation || !secondLocation) return true;
  const direct = distanceBetween(firstLocation, secondLocation);
  return ![...stations.values()].some((station) => {
    if (
      station.line !== line ||
      station.name === first ||
      station.name === second
      ...
      return false;
    const middle = locations.get(station.id);
    if (!middle) return false;
    const fromFirst = distanceBetween(firstLocation, middle);
    const fromSecond = distanceBetween(middle, secondLocation);
    return (
      fromFirst > 0.1 &&
      fromSecond > 0.1 &&
      fromFirst + fromSecond <= direct + 0.25
    );
  });
};
for (const [key, values] of samples) {
  ...
  if (!isPhysicalNeighbor(line, first, second)) continue;
  connect(
    `${line}|${first}`,
    `${line}|${second}`,
...
for (const [name, sameName] of byName) {
  ...
  for (const station of sameName) {
    ...
    station.englishName =
      ...
      metadataByName.get(station.name)?.[0]?.englishName ??
      station.name;
